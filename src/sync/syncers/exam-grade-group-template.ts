...
import {
  ExamGradeGroupTemplate,
  ExamGradeGroupTemplateIndexMapping,
...
const INDEX_NAME: IndexName = "exam_grade_group_template";
...
const universalGenerator = wrapper(
  ...
  async (
    ...
    object: ExamGradeGroupTemplate
  ): Promise<SyncOperation[]> => {
    const { name, space, updated_at } = object;
...
export const onPost = wrapper(
  ...
  async (
    ctx: Context,
    event: QueueEvent<ExamGradeGroupTemplate>
  ): Promise<SyncOperation<ExamGradeGroupTemplateIndexMapping>[]> => {
    ctx.register(event);
